...
def load_and_preprocess_data(csv_path):
    ...
    print("Loading data...")
    df = pd.read_csv(csv_path)[:1000]  # Limit sample size
...
def create_prompt_template(text, risk_score, stock_symbol="STOCK"):
    """Create training prompt template - use risk assessment format"""
    # Use same conversation format as risk_deepseek_deepinfra.py
...
def prepare_dataset(df, tokenizer, max_length=512):
    ...
    for _, row in df.iterrows():
        ...
        stock_symbol = row.get('Stock_symbol', 'STOCK')  # Get stock symbol, use default if not available
    ...
    def tokenize_function(examples):
        ...
        # For language models, labels are input_ids
